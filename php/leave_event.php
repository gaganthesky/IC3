...
if (isset($_GET['U_id']) && isset($_GET['E_id'])) {
    ...
    if ($result) {
        ...
        // Render array from string
        $eventString  = trim($_evtList[0], ',');
        $eventArray = explode(",", $eventString);

        // remove the E_id from the array
        foreach (array_keys($eventArray, $eid) as $key)
        {
        ...
        // echo $eventString;      // DEBUGGING
        // echo $eventArray;       // DEBUGGING

        /////

		$response["uid"]=$uid;
		$response["eid"]=$eid;
		$comma = implode(",",$eventArray);
